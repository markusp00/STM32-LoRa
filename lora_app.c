...
/**
 * @brief Specifies the state of the thermostat
 */
static uint8_t AppThermostatStateOn = RESET;
		...
		if (params->IsMcpsIndication)
		{
			if (appData != NULL)
			{
				...
				if (appData->Buffer != NULL)
				{
					...
					/**
					 * @brief Executes thermostat commands from Cayenne
					 */
					case LORAWAN_THERMOSTAT_PORT:
						if (appData->BufferSize == 4)	// Check if the payload consists of 4 bytes
						{
							AppThermostatStateOn = (appData->Buffer[2] == 0x64);	// Turn on thermostat when the third byte is equal to 0x64
							if (AppThermostatStateOn == RESET)
							{
								APP_LOG(TS_OFF, VLEVEL_M, "THERMOSTAT OFF\r\n");
								HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_RESET);	// Resets GPIO pin B7
							}
							else
							{
								APP_LOG(TS_OFF, VLEVEL_M, "THERMOSTAT ON\r\n");
								HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_SET);	 // Sets GPIO pin B7
							}
						}
						break;
